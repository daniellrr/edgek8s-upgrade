...
check_talosctl_version() {
    ...
    # if [ "$client_version" != "$server_version" ]; then
    #     echo "Mismatch in talosctl binary version. Client: $client_version, Server: $server_version"
    #     exit 1
    # fi

    # if [ "$TALOS_VERSION" != "$server_version" ]; then
    # echo "Mismatch in talosctl binary version. Client: $client_version, Server: $server_version"
    # exit 1
    # fi
}
...
# # Function to calculate the next minor Kubernetes version
# calculate_next_k8s_version() {
#     local current_version=$(get_current_k8s_version)
